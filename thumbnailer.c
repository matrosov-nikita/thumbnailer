...
static int encode_frame(struct Buffer* img, AVFrame* frame)
{
    struct SwsContext* ctx = sws_getContext(frame->width, frame->height,
        frame->format, frame->width, frame->height, AV_PIX_FMT_RGBA,
    ...
    // RGB have one plane
    uint8_t* dst_data[1] = { img->data = malloc(img->size) };
